...
def read_and_write_frames(label, user, sample, sequence_length, input_folder, output_folder):
    ...
    """
    path = os.path.join(input_folder, '{:0>3}_{:0>3}_{:0>3}.avi'.format(label + 1, user + 1, sample + 1))
    #print(path)
    frames = []
    reader = imageio.get_reader(path)
    #length = reader.get_length()
    length = int(VideoFileClip(path).duration * 60)
    print(length)
...
def worker_method(queue, sequence_length, input_folder, output_folder):
    ...
    while True:
        ...
        logger.info('Processed {:0>3}_{:0>3}_{:0>3}.avi'.format(item[0] + 1, item[1] + 1, item[2] + 1))
        queue.task_done()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--sequence_length', type=int, help='The number of frames to extract per video', default=20)
